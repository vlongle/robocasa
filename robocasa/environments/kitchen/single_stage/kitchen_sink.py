...
class TurnSinkSpout(Kitchen):
    ...
    def get_ep_meta(self):
        ...
        # Expose the spout-top geom's live world-frame position as obj_pos —
        # the semantic-guidance target. {prefix}spout_top is the horizontal
        # pipe at the end of the faucet neck (sits at EE height, swings with
        # spout_joint). GrootRoboCasaEnv reads ep_meta["obj_pos"] into
        # state.obj_pos, which _compute_target_point_base_frame then rotates
        # into base frame for the −||pos − target||² attractor.
